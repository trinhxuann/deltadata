...
the Access database inside. If the Access database needs to be
downloaded from a URL or extracted from a compressed file, the file will
be downloaded to Window’s temporary folder (it will be deleted after R
is shut down).
...
```
    ...
    ## [11] "MSysRelationships"          "MSysResources"             
    ## [13] "20mmStations"               "FishCodes"                 
    ## [15] "FishLength"                 "FishSample"                
    ## [17] "Gear"                       "GearCodesLkp"              
    ## [19] "MeterCorrections"           "SampleCode"                
    ## [21] "Station"                    "Survey"                    
    ## [23] "Tow"                        "ZooCodes"                  
    ## [25] "ZooCount"                   "ZooSample"
...
1.  [`plotGPS()`](https://github.com/trinhxuann/deltadata/reference/plotGPS.md):
    ...
    points, and it is recommended that one group be the “Theoretical”
...
```
    ...
    ## Reading from: https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=OSJ,BIR,FCT,BET,HOL,PRI,TRN,RRI,BAC,LPS,DWS,LIB,TOE,M13,HWB,TMS,ANH,MIR,UCS,PCT,C24,C31,PTS,BDL,NSL,VOL,GZL,OH4,TWI,SOI,RYC,HON,MAL,FLT,SJJ,MRZ,MOK,WCI,BLL,IBS&SensorNums=25&dur_code=E&Start=2023-03-12&End=2023-07-08 
    ## Reading from: https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=JER,STI,EMM,CLL,ANC&SensorNums=25&dur_code=H&Start=2023-03-12&End=2023-07-06 
...
```
    ...
    ##                  time station      lat       lon temp  distance
    ## 1 2023-03-15 11:38:00     703 38.04617 -121.7971 10.2 1.9855871
    ## 2 2023-03-15 14:03:00     804 38.01819 -121.7995 11.6 0.1918891
    ## 3 2023-04-10 09:53:00     703 38.04911 -121.7976 14.0 2.1832955
    ## 4 2023-04-24 08:52:00     703 38.04878 -121.7956 15.5 2.1778319
    ## 5 2023-05-08 07:43:00     703 38.04706 -121.7968 15.0 2.0489528
    ## 6 2023-05-08 06:53:00     804 38.01797 -121.7986 15.9 0.2403805
    ##           sensorDescription sensorNumber duration     plot    dataCollection
    ## 1 TEMPERATURE, WATER, DEG F           25    event (TEMP W) DATA XCHG-DWR ISE
    ## 2 TEMPERATURE, WATER, DEG F           25    event (TEMP W) DATA XCHG-DWR ISE
    ## 3 TEMPERATURE, WATER, DEG F           25    event (TEMP W) DATA XCHG-DWR ISE
    ## 4 TEMPERATURE, WATER, DEG F           25    event (TEMP W) DATA XCHG-DWR ISE
    ## 5 TEMPERATURE, WATER, DEG F           25    event (TEMP W) DATA XCHG-DWR ISE
    ## 6 TEMPERATURE, WATER, DEG F           25    event (TEMP W) DATA XCHG-DWR ISE
    ##           dataAvailable cdecGage tempCDEC timeDifference
    ## 1 10/16/2008 to present      ANH 11.55556              7
    ## 2 10/16/2008 to present      ANH 11.50000              3
    ## 3 10/16/2008 to present      ANH 14.27778              7
    ## 4 10/16/2008 to present      ANH 16.44444              7
    ## 5 10/16/2008 to present      ANH 15.94444              2
    ## 6 10/16/2008 to present      ANH 15.88889              7
...
```
    ...
    ## Reading from: https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=MRZ,FLT,TMS,SOI,LIB,MIR,FCT,TRN,RRI,BIR,BAC,WCI&SensorNums=25&dur_code=E&Start=2023-03-13&End=2023-05-10 
    ## Reading from: https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=EMM&SensorNums=25&dur_code=H&Start=2023-05-07&End=2023-05-09 
    ## Reading from: https://cdec.water.ca.gov/dynamicapp/req/CSVDataServlet?Stations=CPP&SensorNums=146&dur_code=H&Start=2023-03-29&End=2023-06-09

    ##   station                time temp      lat       lon distance
    ## 1     915 2023-05-08 08:36:00 15.5 37.94428 -121.5679 1.749597
    ## 2     915 2023-05-08 08:21:00 15.5 37.94428 -121.5679 1.749597
    ## 3     915 2023-05-08 08:49:00 15.5 37.94428 -121.5679 1.749597
    ## 4     914 2023-05-08 11:55:00 16.1 37.97150 -121.5200 2.108294
    ## 5     914 2023-05-08 11:42:00 16.1 37.97150 -121.5200 2.108294
    ## 6     914 2023-05-08 11:20:00 16.1 37.97150 -121.5200 2.108294
    ##           sensorDescription sensorNumber duration     plot dataCollection
    ## 1 TEMPERATURE, WATER, DEG F           25    event (TEMP W)      SATELLITE
    ## 2 TEMPERATURE, WATER, DEG F           25    event (TEMP W)      SATELLITE
    ## 3 TEMPERATURE, WATER, DEG F           25    event (TEMP W)      SATELLITE
    ## 4 TEMPERATURE, WATER, DEG F           25    event (TEMP W)      SATELLITE
    ## 5 TEMPERATURE, WATER, DEG F           25    event (TEMP W)      SATELLITE
    ## 6 TEMPERATURE, WATER, DEG F           25    event (TEMP W)      SATELLITE
    ##           dataAvailable cdecGage tempCDEC timeDifference
    ## 1 04/17/2008 to present      BAC 15.72222              6
    ## 2 04/17/2008 to present      BAC 15.72222              6
    ## 3 04/17/2008 to present      BAC 15.77778              4
    ## 4 03/02/2018 to present      BIR 16.00000              5
    ## 5 03/02/2018 to present      BIR 15.88889              3
